const p = {
  run: async (y) => {
    const i = WA.room.hashParameters.difyApiKey ?? "", l = WA.room.hashParameters.difyBaseUrl || "https://u3coc7ypdwct9ysn.ai-plugin.io", h = `あなたは仮想オフィス「じむしょ村」の受付AI、もちもち美咲です。
...
返答は2〜4文程度、簡潔にまとめてください。`;
    ...
    async function u() {
      var t, o, e;
      const n = await fetch(`${l}/chat/completions`, {
        method: "POST",
        headers: {
          Authorization: `Bearer ${i}`,
          "Content-Type": "application/json"
        },
        body: JSON.stringify({ model: "dify", messages: s })
      });
      if (!n.ok) throw new Error(`Dify API error: ${n.status}`);
      const a = await n.json(), c = (e = (o = (t = a == null ? void 0 : a.choices) == null ? void 0 : t[0]) == null ? void 0 : o.message) == null ? void 0 : e.content;
      if (!c) throw new Error("Dify returned no content: " + JSON.stringify(a));
      return c;
    }
    async function r() {
      ...
      try {
        const t = await u();
        return s.push({ role: "assistant", content: t }), t;
      } finally {
